...
{
    console.log('// ==================== Editing the Case in First Row ========================\n');
    ...
    //Edit Scantype
    const scantype_dropdown = await page.locator('(//div[@class="case-form"]//div[@class="ant-select ant-select-single ant-select-show-arrow ant-select-show-search"])[1]');
    await scantype_dropdown.click();
    //Select "MR"
    //const popupscantype = await page.locator('//div[@class="ant-select-item ant-select-item-option"]//div[normalize-space(text()) = "MR"]').nth(1);
    //const popupscantype = await page.locator('//div[contains(@class, "ant-select-dropdown") and not(contains(@class, "ant-select-dropdown-hidden"))]//div[@class="ant-select-item ant-select-item-option"]//div[normalize-space(text()) = "MR"]');
     const popupscantype = await page.getByRole('dialog').getByText('MR', { exact: true });
    await popupscantype.click();
    console.log("Scantype is edited with the 'MR'.");
    console.log(" ");

    //Edit Body part
    //Close the option which is already there
    ...
    const bodypart = await page.locator('//div[contains(text(),"Brain")]').nth(1);
    await bodypart.click();
    console.log("Bodypart is edited with the 'Brain'.");
    console.log(" ");
